...
def generateSudoer(
    ...
    def runSudoCommand(bashCommand):
        if this_terminal_only:
            ...
            process.communicateStr(
                """do shell script "{0}" with administrator privileges""".format(
                    bashCommand
                )
            )
        else:
    ...
    def attemptSudo(oneLiner, newTerminalCommand=""):
        try:
            ...
            if this_terminal_only:
                print(
                    "The sudo command has failed and we can't launch any more terminals."
                )
                raise
            else:
                print(
                    "Default graphical priviledge escalator has failed for some reason."
                )
                print(
                    "A new terminal will open and you may be prompted for your sudo password."
                )
...
def generateSpeakers(speech_setting):
    ...
    def endSpeaker(expression, failure=False):
        if (
            installing
            and "install" in speech_setting
            or failure
            and "fail" in speech_setting
        ):
            speakerFunc(expression)
...
def generateSpeakerFuncs(speech_setting):
    ...
    def exiter(code=0):
        exitspeaker(
            "Compilation{0}successful.".format(", un" if code != 0 else " "),
            failure=code != 0,
        )
        sys.exit(code)
...
def generateTLMGRFuncs(tlmgr, speaker, sudoFunc):
    ...
    def initializeInstallation():
        ...
        if default_permission:
            ...
            sudoFunc(
                basicCommand,
                """echo \\"This is {0}'s 'install packages on the fly' feature.\\n\\n{1}\\n\\" ; sudo {2}""".format(
                    scriptName, updateInfo, basicCommand
                ),
            )
    ...
    def installPackages(packages):
        ...
        print(
            "{0}: Attempting to install LaTex package(s): {1}".format(
                scriptName, packagesString
            )
        )

        if default_permission:
            process = subprocess.Popen(
                [tlmgr, "install"] + packages, stdin=subprocess.PIPE
            )
            process.wait()
        else:
...
sudo {2}""".format(
                scriptName, packagesString, basicCommand
            )
    ...
    def getSearchResults(preamble, term, strictMatch):
        ...
        print(
            "{0}: Searching repositories for missing {1} {2}".format(
                scriptName, fontOrFile, term
            )
        )

        process = subprocess.Popen(
            [tlmgr, "search", "--global", "--file", term],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        ...
        for line in outList:
            ...
            if line.startswith(preamble) and (
                not strictMatch or line.endswith("/" + term)
            ):
                ...
                results.append(
                    line.split("/")[-3].strip()
                )  # occasionally the package is one more slash before
    ...
    return (
        lambda entry: searchAndInstall(searchFilePackage, entry),
        lambda entry: searchAndInstall(searchFontPackage, entry),
    )
...
def generateCompiler(compiler, arguments, texDoc, exiter):
    def compileTexDoc():
        try:
            process = subprocess.Popen(
                [compiler] + shlex.split(arguments) + [texDoc],
                stdin=sys.stdin,
                stdout=subprocess.PIPE,
            )
            ...
            print(
                "{0}: Unable to start {1}; are you sure it is installed?{2}".format(
                    scriptName,
                    compiler,
                    "  \n\n(Or run "
                    + scriptName
                    + " --help for info on how to choose a different compiler.)"
...
if __name__ == "__main__":
    ...
    parser = optparse.OptionParser(
        ...
        + "Some of its default options can be directly changed in {0}.  For example, the default compiler can be edited on line 4.".format(
            scriptName
        ),
        version="1.2",
    ...
    parser.add_option(
        ...
        dest="compiler",
        metavar="COMPILER",
        help="your LaTeX compiler; defaults to {0}".format(defaultCompiler),
    ...
    parser.add_option(
        ...
        dest="arguments",
        metavar="ARGS",
        help='arguments to pass to compiler; default is: "{0}"'.format(
            defaultArguments
        ),
    ...
    parser.add_option(
        ...
        dest="texlive_bin",
        metavar="LOCATION",
        help="Custom location for the TeX Live bin folder",
        default="",
    )
    parser.add_option(
        "--terminal_only",
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="fail_silently",
        help="If tlmgr cannot be found, compile document anyway.",
        default=False,
    )
    ...
    try:
        ...
        (installFile, installFont) = generateTLMGRFuncs(
            tlmgr_path, installSpeaker, generateSudoer(options.terminal_only)
        )
    except OSError:
        if options.fail_silently:
            ...
            parser.error(
                "{0}: It appears {1} is not installed.  {2}".format(
                    scriptName,
                    tlmgr_path,
                    "Are you sure you have TeX Live 2010 or later?"
                    if tlmgr_path == "tlmgr"
                    else "",
                )
            )
    ...
    while not done:
        ...
        filesSearch = re.findall(
            r"! LaTeX Error: File `([^`']*)' not found", output
        ) + re.findall(r"! I can't find file `([^`']*)'.", output)
        filesSearch = [
            name for name in filesSearch if name != texDoc
        ]  # strips our .tex doc from list of files
        # next most reliable: infers filename from font error
        fontsFileSearch = [
            name + ".tfm" for name in re.findall(r"! Font \\[^=]*=([^\s]*)\s", output)
        ]
        # brute force search for font name in files
        fontsSearch = re.findall(
            r"! Font [^\n]*file\:([^\:\n]*)\:", output
        ) + re.findall(r"! Font \\[^/]*/([^/]*)/", output)
        ...
        try:
            ...
            print(
                "\n{0}: Unable to update; all privilege escalation attempts have failed!".format(
                    scriptName
                )
            )
            print(
                "We've already compiled the .tex document, so there's nothing else to do.\n  Exiting.."
            )
            exitScript(returnCode)
